...
const PROMPT =
  ", On the basis of description please give form in json array  format with form title, form subheading, Form field, form name, placeholder name, and form label,fieldType, field required In Json array format.A valid json array .keep in mind the response should be in a valid json.Dont give any response other than json array ";
        ...
        <div className="grid gap-4 py-2">
          <div className="grid grid-row-4 items-center gap-4">
            <textarea
              ...
              placeholder="Write the desrciption of your form"
            ></textarea>
          </div>
        </div>
        <DialogFooter>
          {recording && <Button onClick={handleOnStop}>Stop</Button>}
          ...
          <Button type="submit" onClick={onGenerateForm} disabled={loading}>
            {" "}
            {loading ? <Loader2 className="animate-spin" /> : "Generate Form"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
